...
module Ai
  ...
  class Demo < Provider
    REPLIES = [
      ...
      "Here's how I'd think about it with Emberline: first, name the smallest version of the outcome that would count as success. Second, sketch the fewest moving parts that could produce it. Third, run that and look at what actually happened instead of what you predicted. Most of the time the gap between two and three is where the real design lives.",
      "I can help with that in Emberline. A useful framing: you're not choosing between options, you're choosing which constraints you're willing to keep. List the three you refuse to give up and the rest of the decision mostly makes itself. Want me to walk through your specific case?"
    ].freeze
    ...
    def stream(messages)
      ...
      reply = PREFIXES.fetch(reply.length % PREFIXES.size) + reply if demo_hint?(last_user)
      reply.split(/(\s+)/).each { |chunk| yield chunk }
    end

    private

    def demo_hint?(last_user)
